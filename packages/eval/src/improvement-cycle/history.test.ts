...
function createPromptFixture<TInput = { name: string }>(
  ...
  return {
    ...
    buildUserPrompt: (input: TInput) => `Hello ${(input as { name: string }).name}!`,
    ...overrides,
...
describe('serializePrompt / deserializePrompt', () => {
  ...
  it('should restore buildUserPrompt that works after deserialization', () => {
    const original = createPromptFixture({
    ...
    const result = deserialized.buildUserPrompt({ name: 'World' })
...
describe('session.addRound', () => {
  ...
  it('should accumulate totalCost correctly', () => {
    ...
    const roundResult2 = createRoundResultFixture({ round: 2, cost: { agent: 0.02, judge: 0.01, improver: 0.003, total: 0.033 } })
    const updatedPrompt = serializePrompt(createPromptFixture({ version: '1.0.1' }))
...
describe('deserializePrompt edge cases', () => {
  ...
  it('should correctly compile valid templates', () => {
    ...
    expect(result.buildUserPrompt).toBeInstanceOf(Function)
    expect(result.buildUserPrompt({ name: 'Test', score: 95 })).toBe(
