...
function createTestPrompt(overrides?: Partial<AgentPrompt<TestInput>>): AgentPrompt<TestInput> {
    return {
        id: 'e2e-test-prompt',
        version: '1.0.0',
        system: 'You are a helpful assistant.',
        userTemplate: 'Query: {{query}}',
        renderUserPrompt: (input: TestInput) => `Query: ${input.query}`,
        ...overrides,
    };
}
...
function createDynamicImprover(suggestionsByRound: Suggestion[][]): Improver {
    ...
    return {
        improve: vi.fn().mockImplementation(async () => {
            const suggestions =
                suggestionsByRound[Math.min(callCount++, suggestionsByRound.length - 1)] ?? [];
            return {
                suggestions,
                metadata: {
...
describe('E2E: Cost Tracking', () => {
    it('should calculate per-round cost breakdown (agent, judge, improver, total)', async () => {
        ...
        expect(value.roundResult.cost.total).toBe(
            value.roundResult.cost.agent +
                value.roundResult.cost.judge +
                value.roundResult.cost.improver
        );
    });
...
describe('E2E: Custom Conditions', () => {
    it('should work with custom condition function', async () => {
        let roundCount = 0;
        const customTerminate = customCondition(async (ctx) => {
