...
export async function executeTestCase<TInput, TOutput>(
  ...
  try {
    resolvedInput = await resolveFilePartsInInput(testCase.input, {
  ...
  if (error) {
    return { kind: 'single-turn', ...testResult, verdicts: [], overallScore: 0, passed: false, judgeMetadata: undefined }
  }
