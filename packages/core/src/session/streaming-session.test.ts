...
import { createStreamingSession, StreamingSession } from './streaming-session';
import {
  createMockModel,
...
describe('createStreamingSession', () => {
  ...
  describe('fileManager', () => {
    ...
    it('should allow file operations through session', async () => {
      ...
      (fileManager.upload as ReturnType<typeof vi.fn>).mockResolvedValue([
        { id: '123', uri: 'gs://bucket/file.pdf', mimeType: 'application/pdf', name: 'file.pdf' },
      ]);
      ...
      const files = await session.fileManager.upload([
        { type: 'file', source: 'path', path: '/tmp/file.pdf', mediaType: 'application/pdf' },
      ]);

      expect(files).toHaveLength(1);
      expect(files[0].uri).toBe('gs://bucket/file.pdf');
    });
  });
  ...
  describe('integration', () => {
    it('should work in a realistic streaming execution flow', async () => {
      ...
      const fileManager = createMockFileManager();
      (fileManager.upload as ReturnType<typeof vi.fn>).mockResolvedValue([
        { id: 'file-1', uri: 'gs://test/file.pdf', mimeType: 'application/pdf', name: 'file.pdf' },
      ]);
      ...
      await session.fileManager.upload([
        { type: 'file', source: 'path', path: '/tmp/doc.pdf', mediaType: 'application/pdf' },
      ]);
