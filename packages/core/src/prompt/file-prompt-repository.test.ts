...
describe('createFilePromptRepository', () => {
    ...
    async function writePromptFile(content: PromptTemplateData): Promise<void> {
    ...
    describe('read', () => {
        describe('with specific version', () => {
            ...
            it('should throw PromptNotFoundError for non-existent prompt', async () => {
                ...
                await expect(repo.read('nonexistent', '1.0.0')).rejects.toThrow(
                    PromptNotFoundError
                );
            });
        });
        ...
        describe('validation', () => {
            it('should throw PromptInvalidFormatError for invalid YAML', async () => {
                ...
                await expect(repo.read('greeting', '1.0.0')).rejects.toThrow(
                    PromptInvalidFormatError
                );
            });
...
`
    ...
    describe('write', () => {
        it('should write prompt to file', async () => {
            ...
            const builder = PromptTemplate.from(data).compile<unknown, { name: string }>();
            expect(builder.renderUserPrompt({ name: 'Test' })).toBe('Hello, Test!');
        });
...
userTemplate: "Hello"
`;
            const readFileMock = vi.fn().mockResolvedValueOnce(yaml1).mockResolvedValueOnce(yaml2);
            const customFs = {
                readFile: readFileMock,
                writeFile: vi.fn(),
